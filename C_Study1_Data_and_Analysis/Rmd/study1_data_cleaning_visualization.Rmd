---
title: "Study 1 Data Cleaning and Exploratory Visualization"
author: "Chisom Obasih"
date: "March 2025"
subtitle: "Cleaning and wrangling raw data of Visual Analog Scale (VAS) task from Study 1 of dissertation, some exploratory visualizations of processed, but un-analyzed data"
...
write_excel_csv(unfiltered_vas_df, file = "data/processed/unfiltered_study1_vas_all.csv")
```
...
ggplot(data = indent_intent_df, aes(x = factor(first_dim_step), y = factor(second_dim_step), color = norm_response)) +
  ...
  facet_wrap(vars(ID))
ggsave(filename = "exploratory_visualization/heatmaps/indent-intent.png", width = 100, height = 80, unit = "cm")
```
...
ggplot(data = reason_risen_df, aes(x = factor(first_dim_step), y = factor(second_dim_step), color = norm_response)) +
  ...
  theme(legend.direction = "horizontal", legend.position = "bottom", text = element_text(size = 50), legend.key.size = unit(4, "cm")) +
  facet_wrap(vars(ID))
ggsave(filename = "exploratory_visualization/heatmaps/reason-risen.png", width = 100, height = 80, unit = "cm")
```
...
ggplot(data = kata_katta_df, aes(x = factor(first_dim_step), y = factor(second_dim_step), color = norm_response)) +
  ...
  theme(legend.direction = "horizontal", legend.position = "bottom", text = element_text(size = 50), legend.key.size = unit(4, "cm")) +
  facet_wrap(vars(ID))
ggsave(filename = "exploratory_visualization/heatmaps/kata-katta.png", width = 100, height = 80, unit = "cm")
```
...
ggplot(data = toru_tooru_df, aes(x = factor(first_dim_step), y = factor(second_dim_step), color = norm_response)) +
  ...
  theme(legend.direction = "horizontal", legend.position = "bottom", text = element_text(size = 50), legend.key.size = unit(4, "cm")) +
  facet_wrap(vars(ID))
ggsave(filename = "exploratory_visualization/heatmaps/toru-tooru.png", width = 100, height = 80, unit = "cm")
```
...
export(filtered_vas, "data/processed/filtered_study1_vas.csv", row.names = F, quote = F)

# save matlab (with 4 columns) as txt file
export(filtered_matlab, "data/processed/filtered_study1_vas_matlab.txt", sep = "\t", row.names = F, quote = F)

# save the averaged version (didn't end up using this version)
export(filtered_matlab_avg, "data/processed/filtered_study1_vas_avg_matlab.txt", sep = "\t", row.names = F, quote = F)
```
------------------------------------------------------------------------
## more visualizations
...
ggsave(filename = "exploratory_visualization/heatmaps/averaged_across_ID_per_lang.png", width = 80, height = 100, unit = "cm")
...
ggsave(filename = "exploratory_visualization/heatmaps/averaged_across_ID_per_pair.png", width = 120, height = 80, unit = "cm")
```
...
for (i in 1:20){
  ggplot(visualization, aes(x = norm_response)) +
    ...
    geom_histogram()
    ggsave(filename = paste0("exploratory_visualization/response_histograms/response_histograms_", i, ".png"))
  }
```
...
#   ggsave(filename = paste0("exploratory_visualization/scatterplots/scatterplot_", unique(visualization$ID)[i], ".png"), width = 10, height = 8, units = "in")
# }
```
