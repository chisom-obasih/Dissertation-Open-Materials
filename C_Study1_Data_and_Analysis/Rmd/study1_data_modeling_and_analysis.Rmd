...
path.to.curvefit.data <-"data/VAS_curvefit_output"
path.to.mld.data <- "data/analyzed_results"

# rotated logistic estimates, convert ID to factor, and add language column, and set factors for language and pair
rl_estimates<-import(paste(path.to.curvefit.data, "study1_vas_rl_curvefit.txt", sep = "/")) %>%
...
MLD_A <- import(paste(path.to.mld.data, "study1_mld-a_scores.csv", sep = "/")) %>%
  mutate(ID = as.factor(ID), gender = as.factor(gender))
MLD_P <- import(paste(path.to.mld.data, "study1_mld-p_scores.csv", sep = "/")) %>%
  mutate(ID = as.factor(ID), gender = as.factor(gender))
MLD_scores <- import(paste(path.to.mld.data, "study1_mld_scores.csv", sep = "/")) %>%
  mutate(ID = as.factor(ID), gender = as.factor(gender))
...
save(list = c("aggregate", # dataframe used in multivariate model (n = 304)
          ...
          file = "Rdata/study1_modeling_and_analysis_workspace.Rdata")
```
...
save(list = c("rl_estimates", # original output from MATLAB
     ...
     file = "Rdata/study1_dataframes_modeling_analysis_results.Rdata")
```
...
# note: file paths are broken and the data this is referencing is obsolete
