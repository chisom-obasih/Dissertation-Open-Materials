---
title: "Study 2 Quantitative Results and Visualization"
author: "Chisom Obasih"
date: "June 2025"
subtitle: "Tables, figures, and graphs after running logistic curvefitter and Bayesian and generalized linear mixed-effects models"
output: 
  pdf_document:
...
load(file = "Rdata/study2_dataframes_modeling_analysis_results.Rdata")
...
save(bright_seven,
     ...
     rt.var,
     rt.plot,
     file = "Rdata/study2_quantitative_results_visualization_workspace.Rdata")
```
...
load(file = "Rdata/study2_quantitative_results_visualization_workspace.Rdata")
```
...
continua.table.1d <- import("../../02_Task_Materials/VAS/planning/continua-table-1d.xlsx")
...
# import spreadsheets
word_pairs_test <- import("../../02_Task_Materials/AX_discrimination/gorilla_spreadsheets/testing_all_groups.csv") %>%
...
word_pairs_train <- import("../../02_Task_Materials/AX_discrimination/gorilla_spreadsheets/training_high_LD.csv") %>%
  filter(!is.na(randomize_language_blocks)) %>%
  select(word_pair, word_A, word_B, transliteration_A:task) %>%
  distinct() %>%
  select(word_pair, transliteration_A:transliteration_B, orthography_A:gloss_B, phoneme_quality, segment_type, task)
